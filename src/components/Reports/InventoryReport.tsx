...
const InventoryReport: React.FunctionComponent<IProps> = ({
  user,
  store,
  setStore,
  stores,
  ...
  const refreshAnalytics = (): void => {
    ...
    // console.log({ year, month, manufacturer: user.manufacturer_id, retailLocation, token })
    reportService
      ...
      <Card.Body>
                  ...
                  {category.products.map((product) => (
                    <tr key={product._id}>
                      ...
                      {Object.values(product.dates).map(
                        (detail: any, index) => (
                          <React.Fragment key={index}>
                            <td>{detail.inventory.stock.store}</td>
                            <td>{detail.value}</td>
                          </React.Fragment>
                        )
                      )}
        ...
        {!isLoading && !salesData.data.length ? (
          <div className="d-flex justify-content-center my-4">
            {store ? (
              <span>No inventory report available</span>
            ) : (
              <span>Set filters and refresh to see inventory report</span>
            )}
          </div>
        ) : null}
